...
def login_post():
    ...
    host = connexion.request.host_url
    return (
        {
            "X-Auth-Token": token,
...
def common_get_samples(token_info, scope_id, task):
    if task != "404":
        return (constants.TEST_CASE_LIST, 200)

    return ({"message": "Test cases not found", "code": "client_error"}, 404)

def course_get_samples(token_info, course_id, task):
    return common_get_samples(token_info, course_id, task)

def contest_get_samples(token_info, contest_id, task):
    return common_get_samples(token_info, contest_id, task)
