...
pub fn get_template(ui: &mut Ui<impl RP>, params: command::Template) -> Result<()> {
    ...
    if service::file_exists(&ui.res, &template_response.file_name) {
        let overwrite_message = format!("A file {} already exists. Are you sure you want to overwrite the file with the downloaded template (yes/No)? ", &template_response.file_name);
